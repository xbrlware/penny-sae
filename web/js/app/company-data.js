...
App.DetailModel = Ember.Object.extend({
  ...
  tradingHalt: function () {
    ...
    if (Object.keys(th).length > 1) {
      var out = [];
      for (var i = 0; i < th.date.length; i++) {
        var url = th.url[i];
  ...
  spikesTable: function () {
    ...
    if (s !== undefined) {
      s = s[0];
      var tab = [];
      for (var i = 0; i < s.spike_size.length; i++) {
        var obj = {
  ...
  financialsTable: function () {
    ...
    if (Object.keys(d).length > 1) {
      var tab = [];
      for (var i = 0; i < d.balance_sheet_date.length; i++) {
        var obj = { };
  ...
  delinquencyTable: function () {
    ...
    if (d !== undefined) {
      if (Object.keys(d).length > 1) {
        ...
        for (var i = 0; i < d.date_of_filing.length; i++) {
          var obj = { };
