...
App.LeadershipRoute = Ember.Route.extend({
  model: function () {
    ...
    return fetch_leadership({'cik': cik})
  },
...
App.LeadershipView = Ember.View.extend({
  ...
  make_chart: function (data, posNames) {
    ...
    var x0 = d3.scale.ordinal().rangeRoundBands([0, height], .1)
    ...
    state.selectAll('rect')
      ...
      .attr('width', function (d) {var h = y(d.value.stop) - y(d.value.start)
        if (isNaN(h)) {h = 0} else if (h === 0) {h = 5};  return h;})
