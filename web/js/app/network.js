...
/* global Ember, App, _, $jit, fetch, gconfig, config, implementIcons, setRedFlags */

// Ownership Network Visualization
// Aesthetics for network visualization

function createNetworkAssociates (networkCenter, NetworkAssociates) {
  if (NetworkAssociates.length > 0) {
    return _.chain(NetworkAssociates).filter(function (x) { return x.id; }).map(function (x) {
      // console.log('network_associate', x)
      var adj = networkCenter._source.adjacencies.findBy('nodeTo', x.id);
...
function addNode (con, action, cik, rfClean, isNew, that, node, rgraph) {
  return fetch({
    ...
    callback: function (center) {
      if (center.hits.hits.length > 0) {
        var networkCenter = center.hits.hits.findBy('_index', config.NETWORK_INDEX);
        var origAdj = networkCenter._source.adjacencies;
        if (networkCenter !== undefined) {
          if (action === 'initial') {
            con.set('isLoading', true); // *
          }

          networkCenter.companies = center.hits.hits.findBy('_index', config.COMPANY_INDEX);

          fetch({
            endpoint: 'fetch_companies',
            index: config.NETWORK_INDEX,
            query_type: 'networkQuery_neighbors',
            query_args: {'adj': networkCenter._source.adjacencies},
            rf: rfClean,
            callback: function (neighbors) {
              var NetworkAssociates = createNetworkAssociates(
                networkCenter,
                _.sortBy(
                  _.pluck(
              ...
              if (action === 'initial') {
                con.set('orig_NetworkAssociates', NetworkAssociates);
                con.set('NetworkAssociates', NetworkAssociates);
              }
              var companies = _.filter(neighbors.hits.hits, function (hit) {
                return hit._index === config.COMPANY_INDEX;
              });
              var networkNeighbors = _.filter(neighbors.hits.hits, function (hit) {
                return hit._index === config.NETWORK_INDEX;
              });
              _.map(networkNeighbors, function (neighbor) {
                neighbor.companies = companies.findBy('_id', neighbor._id);
              });

              // Find neighbors that are either not NER or not hidden
              var subNetworkNeighbors = _.filter(networkNeighbors, function (neighbor) {
                // Get node data from edge...
                var equiv = _.filter(NetworkAssociates, function (x) {
                  return x.id === neighbor._id || smartParseInt(x.id) === neighbor._id;
                })[0];
              ...
              if (action === 'update') {
                ...
                if (subNetworkNeighbors.length > 0) {
                  initElasticBig(
                    networkCenter,
                    subNetworkNeighbors,
                    rfClean,
                    function (json) {
                      var rgraph = isNew === true ? makeRGraph(con, 'main-infovis', rfClean) : con.get('rgraph_object');
                      rgraph.loadJSON(json);
                      rgraph.refresh();

                      con.set('rgraph_origin', networkCenter);
                      con.set('origAdj', origAdj);
                      ...
                      function getAlpha (node, hideTerminal, hideNer) {
                        if (hideTerminal && node.data['terminal']) {
                          return 0;
                        } else if (hideNer && node.data['ner']) {
                          return 0;
                        } else {
                          return 1;
                        }
                      ...
                      rgraph.graph.eachNode(function (node) {
                        node.setData('alpha', getAlpha(node, con.get('hideTerminal'), con.get('hideNer')), 'end');
                      });
                      rgraph.fx.animate({
...
function redFlagIndividuals (args) {
  Ember.$.ajax({
    type: 'POST',
    contentType: 'application/json',
    dataType: 'json',
    url: 'redFlagIndividuals',
    data: JSON.stringify({
    ...
    return new Ember.RSVP.Promise(function (resolve, reject) {
      fetch({
        ...
        callback: function (data) {
          ...
          if (hits.length > 0) {
            Ember.$.each(adj, function (key, value) {
              ...
              str += '<div class="col-lg-6 col-md-6"><div class="dummy100"></div><a class="thumbnail" href="index.html#/o/detail/' + cik + '">' + content + '</a></div>';
            });
            str += '</div></div></center>';
          }
          resolve(str);
      ...
      fetch({
        ...
        callback: function (data) {
          ...
          if (hit.length > 0) {
            ...
            content += '<p>';
            if (sic !== undefined) { content += sic + '<br>'; }
            if (state !== undefined) { content += state; }
            content += '</p>';

            var cik = parseInt(node.id);
            if (content !== '') {
              str += '<div class="container"><div class="row"><div class="col-sm-9"><div class="dummy25"></div><a class="thumbnail" href="index.html#/o/detail/' + cik + '">' + content + '</a></div></div></div>';
            }
          }
...
App.NetView = Ember.View.extend({
  ...
  didInsertElement: function () {
    this.get('controller').set('hideTerminal', gconfig.DEFAULT_HIDE_TERMINAL);
    this.get('controller').set('hideNer', gconfig.DEFAULT_HIDE_NER);
  ...
  initRGraph: function (that, isNew) {
    ...
    /* rfClean isn't working for some reason... swap this later */
    if (rf !== undefined) {
      var exists = { };
        ...
        if (key !== 'exists' && key !== 'toggles') {
          ...
          if (rf[key] !== undefined) {
            Object.keys(rf[key]).map(function (innerKey) {
              if (rf[key][innerKey] !== undefined) { exists[key] = true; }
            });
          }
        }
      });
    ...
    return addNode(con, 'initial', that.data.get('content').cik, rfClean, isNew, that, undefined, undefined);
  },
  ...
  addButton: function (con, rgraph) {
    ...
    button.onclick = function () {
      con.toggleProperty('hideTerminal');
      if (con.get('hideTerminal')) {
        Ember.$(this).context.value = 'Show Terminal Nodes';
      } else {
        Ember.$(this).context.value = 'Hide Terminal Nodes';
      }
      rgraph.graph.eachNode(function (node) {
        node.setData('alpha', getAlpha(node, con.get('hideTerminal'), con.get('hideNer')), 'end');
      });
    ...
    var nerButton = $jit.id('toggle-ner');
    nerButton.onclick = function () {
      con.toggleProperty('hideNer');
      if (con.get('hideNer')) {
        Ember.$(this).context.value = 'Show Augmented Network';
      } else {
        Ember.$(this).context.value = 'Hide Augmented Network';
      }
      rgraph.graph.eachNode(function (node) {
        node.setData('alpha', getAlpha(node, con.get('hideTerminal'), con.get('hideNer')), 'end');
      });
...
function initElasticBig (networkCenter, neibs, rfClean, callback) {
  // Prevent unselected named entities from plotting
  networkCenter._source.adjacencies = _.filter(networkCenter._source.adjacencies, function (adj) {
    return _.where(neibs, {'_id': smartParseInt(adj.nodeTo) + ''}).length > 0;
  });

  var network = [];
  var ths = networkCenter._source;

  if (networkCenter.companies !== undefined) {
    var crf = setRedFlags(rfClean, networkCenter.companies.fields);
  ...
  var allCiks = _.uniq(_.pluck(neibs, '_id'));

  redFlagIndividuals({
    query_args: {'allCiks': allCiks},
    rf: rfClean,
    callback: function (companyData) {
      fetch({
        ...
        query_args: {'allCiks': allCiks},
        rf: rfClean,
        callback: function (adjResponse) {
          console.log('adjResponse ---> ', adjResponse);
          var adjs = _.filter(adjResponse.hits.hits, function (x) {
            return x._index === config.NETWORK_INDEX;
          });

          _.map(neibs, function (neib) {
            var adjMatch = adjs.findBy('_id', neib._id);
            neib._source.adjacencies = adjMatch._source.adjacencies;
          });

          Ember.$.each(neibs, function (index, n) {
            // Find neighbor in network adjacencies
            var src = JSON.parse(JSON.stringify(n._source));
            var edge = networkCenter._source.adjacencies.findBy('nodeTo', src.id);

            // Fix relationship of neighbor
            src.data['relationship'] = edge.data.relationship !== undefined ? edge.data.relationship : 'uknown';

            // Find adjacencies to neighbors
            var newAdjacencies = [];
            var unconn = 0;
            src.adjacencies.forEach(function (a) {
              var foundNew = neibs.findBy('_source.id', a.nodeTo) === undefined ? false : true; // eslint-disable-line  no-unneeded-ternary

              var foundOld = network.findBy('id', a.nodeTo) === undefined ? false : true; // eslint-disable-line  no-unneeded-ternary
              if (foundNew || foundOld) {
                newAdjacencies.push(a);
              } else {
                unconn++;
              }
            });
            src.adjacencies = newAdjacencies;
            ...
            if (n.companies !== undefined) {
              var crf = setRedFlags(rfClean, n.companies.fields);
              src.data['totalRedFlags'] = crf.total;
            } else {
              companyData.map(function (x) {
                if (x.cik === n._id) {
                  if (src.id === undefined) {
                    src.id = x.cik;
                  }
...
function updateElastic (neibs, networkCenter, centerNode, rgraph, rfClean, callback) {
  networkCenter._source.adjacencies = _.filter(networkCenter._source.adjacencies, function (adj) {
    return _.where(neibs, {'_id': smartParseInt(adj.nodeTo) + ''}).length > 0;
  });

  centerNode.data['invisible'] = 0;
  var allCiks = _.pluck(neibs, '_id');
          ...
          Ember.$.each(neibs, function (index, nn) {
            ...
            if (n === undefined) {
              ...
              if (nn.companies !== undefined) {
                counter++;
                var crf = setRedFlags(rfClean, nn.companies.fields);
            ...
            src.adjacencies.forEach(function (a) {
              ...
              if (to !== undefined) {
                ...
                rgraph.graph.addAdjacence(n, to, a.data);
              } else {
                unconn++;
              }
            });
...
function makeRGraph (con, into, rfClean) {
  var rgraph = new $jit.RGraph({
    ...
    Events: {
      ...
      onClick: function (node, eventInfo, e) {
        if (node) {
          node.setData('dim', gconfig.STANDARD_NODE_SIZE);
          addNode(con, 'update', node.id, rfClean, false, undefined, node, rgraph);
          rgraph.plot();
        }
      },
    ...
    onBeforePlotNode: function (node) {
      // Resize node
      if (node.data['$dim'] !== gconfig.DRAG_NODE_SIZE) { node.data['$dim'] = gconfig.STANDARD_NODE_SIZE; }

      // Hide terminal nodes
      if (con.get('hideTerminal') && node.data['terminal']) {
        node.data['$alpha'] = 0;
      }

      // Hide Augmented nodes
      if (con.get('hideNer') && node.data['ner']) {
        node.data['$alpha'] = 0;
      }
