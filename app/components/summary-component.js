// app/components/summary-component.js
...
export default Ember.Component.extend(SearchMixin, {
  ...
  chartObj: ChartObject.create(),
  summaryModel: undefined,

  init: function () {
    this._super(...arguments);
    const _this = this;
    this.get('fetchData')('topic_summary', {query: this.get('searchCompanies.searchTerm') || ''}).then(function (response) {
      _this.set('summaryModel', response);
      _this.drawDateHistogram();
      _this.drawSicHistogram();
    });
  },
  ...
  drawDateHistogram: function () {
    var data = _.map(this.get('summaryModel.date_histogram'), function (d) {
  ...
  drawSicHistogram: function () {
    var data = _.map(this.get('summaryModel.sic_histogram'), function (d) {
