// app/components/board-component.js
/* global d3, sessionStorage */

import Ember from 'ember';
import _ from 'underscore';
import ChartObject from '../objects/chart-object';
...
export default Ember.Component.extend(SearchMixin, {
  ...
  sentiment: undefined,
  numOfPosters: undefined,
  filteredData: undefined,
  dateFilter: [new Date(Gconfig.DEFAULT_DATE_FILTER[0]), new Date(Gconfig.DEFAULT_DATE_FILTER[1])],
  searchMixin: undefined,

  init: function () {
    this._super(...arguments);
    this.set('cik', this.get('detail'));
    this.set('chartObject', ChartObject.create());
    this.set('isLoading', true);
    this.set('filteredData', []);
    this.set('isData', true);
    this.set('searchTerm', '');
    this.set('sentiment', 'na');
    this.set('numOfPosters', 10);
    this.set('searchMixin', this.get('SearchMixin'));
  ...
  didInsertElement () {
    const _this = this;
    let cik = this.get('detail');
    this.fetchData('cik2name', {'cik': cik}).then(function (cData) {
      _this.fetchData('board', {'cik': cik, 'ticker': cData.ticker, 'date_filter': _this.get('dateFilter'), 'sentiment': {type: 'na', score: 0.5}}).then(function (response) {
        _this.set('boardModel', response);
        _this.set('filteredData', _.map(response.data, function (x) {
          x.date = new Date(x.date);
          return x;
        }));
        _this.set('splitByFilter', []);
        _this.set('pageCount', 1);
        _this.set('routeName', 'board');
        // this.set('selectionIds', params.params[this.get('routeName')].ids);
        // this.set(this.get('routeName') + '_filter', params.params[this.get('routeName')].ids);
        _this.set('isLoading', false);

        if (!response.pvData && !_this.get('boardModel').data) { _this.set('isData', false); }
  ...
  rtDraw: function () {
    ...
    var data = this.get('boardModel.ptData');
    var pvdata = this.get('boardModel.pvData');
    ...
    this.chartObject.makeBarChart(this.postsChart, forumData, brushDomain);
    this.chartObject.makeClose(this.priceChart, pvData, brushDomain);
    this.chartObject.makeBarChart(this.volumeChart, pvData, brushDomain);
  ...
  postFilteredData: function () {
    /* populates forum posts with data from filteredData */
    /* watches these properties: filteredData, dateFilter, pageCount */
    var data = _.sortBy(this.get('filteredData'), 'date').reverse();
    ...
    return r;
  }.property('filteredData', 'pageCount'),
  ...
  redraw: function () {
    ...
    // call to ES
    this.fetchData('redraw', data).then(function (response) {
      _this.set('splitByFilter', []);
      _this.set('pageCount', 1);
      _this.set('boardModel.tlData', response.tlData);
      _this.set('boardModel.data', response.data);
      _this.set('filteredData', _.map(response.data, function (x) {
        x.date = new Date(x.date);
        return x;
      }));
  ...
  renderX: function () {
     ...
     */
    var _this = this;
    var model = this.get('boardModel.tlData');
    ...
    Ember.run.next(function () {
      _.map(timeseries, function (t) {
        _this.chartObject.makeTimeSeries(t, {
          'xmin': xmin,
          'xmax': xmax,
          'ymax': t.max,
          'ymin': 0
  ...
  renderGauges: function () {
    ...
    var _this = this;
    var data = this.get('boardModel.tlData');

    Ember.run.next(function () {
      _.map(data, function (x) {
        return _this.chartObject.drawGauge('#gauge-' + x.id, x.pred_data);
      });
    });
  ...
  toggleSentiment: function (st) {
    ...
    this.fetchData('cik2name', {'cik': cik}).then(function (cData) {
      _this.fetchData('board', {'cik': cik, 'ticker': cData.ticker, 'date_filter': _this.get('dateFilter'), 'sentiment': {type: st, score: 0.5}}).then(function (response) {
        _this.set('boardModel', response);
        _this.set('filteredData', _.map(response.data, function (x) {
          x.date = new Date(x.date);
          return x;
  ...
  actions: {
    ...
    topXClicked: function (id) {
      ...
      if (this.get('splitByFilter').length) {
        this.fetchData('user', {cik: cik, users: this.get('splitByFilter'), date_filter: this.get('dateFilter'), search_term: this.get('searchTerm'), sentiment: {type: _this.get('sentiment'), score: 0.5}}).then(function (response) {
          _this.set('filteredData', _.map(response, function (x) {
            x.date = new Date(x.date);
            return x;
          }));
        });
      } else {
        this.set('filteredData', this.get('boardModel.data'));
      }
    },
